...
otpverificationforforgetpassword(context, otp) async {
  var headers = {'Content-Type': 'application/json'};
  showloadingDialog("Loading", context);
  var request = http.Request('POST',
      Uri.parse('https://staging-api.meribilty.com/api/verify-forgot-password'));
  ...
  if (response.statusCode == 200) {
    ...
    if (responseMap['status'] == true) {
      Navigator.pushReplacement(      
          context,
          MaterialPageRoute(
              builder: (_) => PasswordSet(phoneNumber: responseMap['phone'])));
    }
  } else {
    Navigator.pop(context);
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
